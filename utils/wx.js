	...
	obj.prototype = {
		...
		payRequest: function (payAppId, payAppKey, partnerId, partnerKey, productName, orderNum, totalPrice, clientIp, notifyUrl, callback) {
			...
			WeixinJSBridge.invoke('getBrandWCPayRequest', {
				...
				"nonceStr": that._getNonceStr(),
				"package": that._getPackage(partnerId, partnerKey, productName, orderNum, totalPrice, clientIp, notifyUrl),
				"signType": "SHA1",
				"paySign": that._getSign(payAppId,payAppKey)
			}, function (res) {
		...
		_onBridgeReady: function(){
			WeixinJSBridge.on('menu:share:appmessage',
				function (argv) {
					...
					function (res) {
						if (that.shareCallback) that.shareCallback(res);
					})
				}
			);
			...
			WeixinJSBridge.on('menu:share:timeline',
				function (argv) {
					WeixinJSBridge.invoke('shareTimeline', {
						...
						"img_height": "640",
						"link": that.shareParam.ShareLink,
						"title": that.shareParam.ShareDesc,
						"desc": ""
					},
					function (res) {
						if (that.shareCallback) that.shareCallback(res);
