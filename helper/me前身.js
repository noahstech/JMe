var PageSwitcher = PS = (function (config) {
	var that;
	var obj = function () {
		that = this;
	...
	obj.prototype = {
		...
		require: function (ctrlName, fn) {
			if (window[ctrlName]) throw new Error("PS中已经注册了名为" + ctrlName + "的控制器");
			if (!fn || !fn.ctrl) throw new Error("PS.require方法中的fn参数对象中需要提供ctrl方法");
